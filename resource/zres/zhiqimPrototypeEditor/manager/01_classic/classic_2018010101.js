...
{//点击二级栏目
    ...
    $tab = Z('<li class="iframenav-tab-item active" id="'+ tabId +'"><span>'+ clickText +'</span><i class="z-font z-error"></i></li>')
        .appendTo($tabList);
    $tab.on("click", Zmr.selectIframeTab);
...
Zmr.selectIframeTab = function(event)
{// 选项卡点击选择
    var $tab = Z(Z.E.current(event));
    var tabId = $tab[0].id;
    var iframeId = tabId.replace("tab_", "iframe_");
    $tab.addClass("active").siblings(".iframenav-tab-item").removeClass("active");
    Z("#"+iframeId).addClass("active").siblings(".iframenav-cont-item").removeClass("active");
    Zmr.setTranslateForShowActive();
}

Zmr.selectIframeTabIndex = function()
{// 首页选项卡
    Z(".iframenav-tab-item:first-child").addClass("active").siblings(".iframenav-tab-item").removeClass("active");
    Z(".iframenav-cont-item:first-child").addClass("active").siblings(".iframenav-cont-item").removeClass("active");
}
...
Zmr.closeIframeTabActive = function(typeNum)
{
    ...
    else if (typeNum === 0)  //所有
        $$tab.each(function($tab){idList.push($tab.id.replace("tab_",""));});
    else //其他
        $$tab.each(function($tab){
            if ($active[0] !== $tab)
                idList.push($tab.id.replace("tab_",""));
        });
