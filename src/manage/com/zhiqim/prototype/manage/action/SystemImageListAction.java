 ...
 */
public class SystemImageListAction extends StdSwitchAction
{
    ...
    protected void list(HttpRequest request) throws Exception
    {
        ...
        sel.addMust("parent", "image");
        sel.addOrderbyAsc("pid");
        List<ElementClassification> imageClsList = ORM.get(ZTable.class).list(ElementClassification.class, sel);

        // 重组为 code->eleCls hashMap
        HashMap<String, ElementClassification> clsCodeMap = new HashMap<String, ElementClassification>();
        for (int i=0;i<imageClsList.size();i++)
        {
            ElementClassification eleCls = imageClsList.get(i);
        ...
        request.setAttribute("imageClsList", imageClsList);
